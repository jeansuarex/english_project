...
export default function Definitions() {
  ...
  const [currentEntry, setCurrentEntry] = useState<DefinitionEntry | null>(null)
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch('/definitions.txt')
        ...
        const all = text.split('\n').map(line => line.trim()).filter(Boolean).map(line => {
          ...
          return {
            definition: parts[0],
            correct: parts[1],
            options: shuffleArray([parts[1], parts[2], parts[3], parts[4]]),
          }
        })
        setEntries(shuffleArray(all))
  ...
  useEffect(() => {
    if (gameState !== 'playing' || entries.length === 0) return

    const totalRounds = Math.min(rounds, entries.length)
    if (currentRound >= totalRounds) {
      setGameState('finished')
      getToken().then(token => {
        fetch('/api/progress/game-session', {
          method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ gameType: 'definitions', correct: score.correct, wrong: score.wrong, totalRounds }),
        })
      })
      return
    }
    ...
    const entry = entries[currentRound]
  ...
  }, [gameState, currentRound, entries, rounds, stopTimers])
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--sage)', fontSize: '15px' }}>Loading definitions...</p>
        </div>
      ...
      <div style={{
        ...
        {!isFinished ? (
          <div style={{
            ...
            <CountdownTimer timeLeft={timeLeft} totalTime={15} />
